...
p <- plot_df %>%
  group_by(album_name, release_date, track_mode_recoded, track_key_recoded) %>%
  summarize(n = n()) %>%
  ggplot(
    aes(
      x = track_key_recoded, 
  ...
  facet_wrap(~ reorder(album_name, release_date), nrow = 2) +
  scale_y_continuous(
    limits = c(-2, 8),
    expand = c(0, 0),
    breaks = c(0, 2, 4, 6, 8)
  ...
  theme(
    ...
    axis.text.x = element_text(family = 'Arial Unicode MS', size = 8),
